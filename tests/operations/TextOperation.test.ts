...
describe('TextOperation shouldBeComposedWith', () => {
    test('first natural test', () => {
...
describe('TextOperation transform', () => {
    test('natural test insert', () => {
        let a = new TextOperation().insert('2');
        let b = new TextOperation().insert('3');

        let [transformA, transformB] = TextOperation.transform(a, b);
        expect(transformB.apply(a.apply(''))).toBe(transformA.apply(b.apply('')));
    });

    test('natural test same length', () => {
        let doc = '1';

        let a = new TextOperation().delete(1);
        let b = new TextOperation().delete(1);
        let [transformA, transformB] = TextOperation.transform(a, b);
        expect(transformB.apply(a.apply(doc))).toBe(transformA.apply(b.apply(doc)));

        a = new TextOperation().retain(1);
        b = new TextOperation().delete(1);
        [transformA, transformB] = TextOperation.transform(a, b);
        expect(transformB.apply(a.apply(doc))).toBe(transformA.apply(b.apply(doc)));

        a = new TextOperation().delete(1);
        b = new TextOperation().retain(1);
        [transformA, transformB] = TextOperation.transform(a, b);
        expect(transformB.apply(a.apply(doc))).toBe(transformA.apply(b.apply(doc)));
    ...
    test('natural test different lengths', () => {
        let doc = '123';

        let a = new TextOperation().delete(2).delete(1);
        let b = new TextOperation().delete(1).delete(2);
        let [transformA, transformB] = TextOperation.transform(a, b);
        expect(transformB.apply(a.apply(doc))).toBe(transformA.apply(b.apply(doc)));
        [transformA, transformB] = TextOperation.transform(b, a);
        expect(transformA.apply(b.apply(doc))).toBe(transformB.apply(a.apply(doc)));

        a = new TextOperation().retain(2).delete(1);
        b = new TextOperation().delete(1).retain(2);
        [transformA, transformB] = TextOperation.transform(a, b);
        expect(transformB.apply(a.apply(doc))).toBe(transformA.apply(b.apply(doc)));
        [transformB, transformA] = TextOperation.transform(b, a);
        expect(transformA.apply(b.apply(doc))).toBe(transformB.apply(a.apply(doc)));

        a = new TextOperation().delete(2).retain(1);
        b = new TextOperation().retain(1).delete(2);
        [transformA, transformB] = TextOperation.transform(a, b);
        expect(transformB.apply(a.apply(doc))).toBe(transformA.apply(b.apply(doc)));
        [transformB, transformA] = TextOperation.transform(b, a);
        expect(transformA.apply(b.apply(doc))).toBe(transformB.apply(a.apply(doc)));

        a = new TextOperation().retain(2).retain(1);
        b = new TextOperation().retain(1).retain(2);
        [transformA, transformB] = TextOperation.transform(a, b);
        expect(transformB.apply(a.apply(doc))).toBe(transformA.apply(b.apply(doc)));
        [transformB, transformA] = TextOperation.transform(b, a);
        expect(transformA.apply(b.apply(doc))).toBe(transformB.apply(a.apply(doc)));
    ...
    test('errors test', () => {
        let a = new TextOperation().retain(1);
        let b = new TextOperation().insert('a');

        expect(() => TextOperation.transform(a, b)).toThrow(/base length/);
        b.baseLength = 1;
        expect(() => TextOperation.transform(a, b)).toThrow(/second operation is too short/);
        expect(() => TextOperation.transform(b, a)).toThrow(/first operation is too short/);
        ...
        expect(() => TextOperation.transform(a, b)).toThrow(/not compatible/);
    });
